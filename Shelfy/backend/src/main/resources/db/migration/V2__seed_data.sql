...
-- =============================================================================
-- 순환 참조 해결 순서:
-- 1. users 삽입 (profile_image_id = NULL)
-- 2. files 삽입 (uploader_id → users 참조 가능)
-- 3. users.profile_image_id UPDATE (fk_users_profile_image는 DEFERRABLE)
-- =============================================================================

-- 2. users (profile_image_id = NULL로 먼저 삽입)
INSERT INTO users (
...
VALUES
    (1,  'alice@shelfy.dev',  '$2b$12$jBHpRdS3spi3NsjQGCsVquMQ84Rhn2jZbX1pG0V.HxZ7pjEVU.dvW',
     'alice_design',   '포토샵·일러스트 템플릿 전문 셀러입니다.',
     NULL, TRUE, TRUE, TRUE, TRUE,  0, NULL, NULL,
     '2026-01-01 09:00:00+09', '2026-01-01 09:00:00+09'),
    (2,  'bob@shelfy.dev',    '$2b$12$jBHpRdS3spi3NsjQGCsVquMQ84Rhn2jZbX1pG0V.HxZ7pjEVU.dvW',
     'bob_coder',      '개발 강의와 코드 템플릿을 공유합니다.',
     NULL, TRUE, TRUE, TRUE, FALSE, 0, NULL, NULL,
     '2026-01-02 09:00:00+09', '2026-01-02 09:00:00+09'),
    (3,  'carol@shelfy.dev',  '$2b$12$jBHpRdS3spi3NsjQGCsVquMQ84Rhn2jZbX1pG0V.HxZ7pjEVU.dvW',
     'carol_photo',    '풍경 사진 작가. 라이트룸 프리셋도 판매해요.',
     NULL, TRUE, TRUE, TRUE, TRUE,  0, NULL, NULL,
     '2026-01-03 09:00:00+09', '2026-01-03 09:00:00+09'),
    (4,  'dave@shelfy.dev',   '$2b$12$jBHpRdS3spi3NsjQGCsVquMQ84Rhn2jZbX1pG0V.HxZ7pjEVU.dvW',
     'dave_music',     '인디 뮤지션. 루프팩과 샘플팩을 제작합니다.',
     NULL, TRUE, TRUE, TRUE, FALSE, 0, NULL, NULL,
     '2026-01-04 09:00:00+09', '2026-01-04 09:00:00+09'),
    (5,  'eve@shelfy.dev',    '$2b$12$jBHpRdS3spi3NsjQGCsVquMQ84Rhn2jZbX1pG0V.HxZ7pjEVU.dvW',
     'eve_ux',         'UX 디자이너. Figma 컴포넌트 키트 전문.',
     NULL, TRUE, TRUE, TRUE, TRUE,  0, NULL, NULL,
     '2026-01-05 09:00:00+09', '2026-01-05 09:00:00+09'),
    (6,  'frank@shelfy.dev',  '$2b$12$jBHpRdS3spi3NsjQGCsVquMQ84Rhn2jZbX1pG0V.HxZ7pjEVU.dvW',
     'frank_dev',      '풀스택 개발자. SaaS 보일러플레이트 판매.',
     NULL, TRUE, TRUE, TRUE, FALSE, 0, NULL, NULL,
     '2026-01-06 09:00:00+09', '2026-01-06 09:00:00+09'),
    (7,  'grace@shelfy.dev',  '$2b$12$jBHpRdS3spi3NsjQGCsVquMQ84Rhn2jZbX1pG0V.HxZ7pjEVU.dvW',
     'grace_writer',   '콘텐츠 마케터. 카피라이팅 템플릿 작가.',
     NULL, TRUE, TRUE, TRUE, TRUE,  0, NULL, NULL,
     '2026-01-07 09:00:00+09', '2026-01-07 09:00:00+09'),
    (8,  'henry@shelfy.dev',  '$2b$12$jBHpRdS3spi3NsjQGCsVquMQ84Rhn2jZbX1pG0V.HxZ7pjEVU.dvW',
     'henry_buyer',    NULL, NULL, TRUE, TRUE, TRUE, FALSE, 0, NULL, NULL,
     '2026-01-08 09:00:00+09', '2026-01-08 09:00:00+09'),
    (9,  'iris@shelfy.dev',   '$2b$12$jBHpRdS3spi3NsjQGCsVquMQ84Rhn2jZbX1pG0V.HxZ7pjEVU.dvW',
     'iris_sub',       '구독으로 콘텐츠를 즐기는 사용자.',
     NULL, TRUE, TRUE, TRUE, TRUE,  0, NULL, NULL,
     '2026-01-09 09:00:00+09', '2026-01-09 09:00:00+09'),
    (10, 'jack@shelfy.dev',   '$2b$12$jBHpRdS3spi3NsjQGCsVquMQ84Rhn2jZbX1pG0V.HxZ7pjEVU.dvW',
     'jack_unverified', NULL, NULL, FALSE, TRUE, TRUE, FALSE, 0, NULL, NULL,
     '2026-01-10 09:00:00+09', '2026-01-10 09:00:00+09');

SELECT setval('users_id_seq', 10, true);

-- =============================================================================
-- 1. files (프로필 이미지, 상품 이미지 — users 삽입 후 등록)
-- =============================================================================
...
-- users.profile_image_id 업데이트 (files 삽입 후)
UPDATE users SET profile_image_id = 1  WHERE id = 1;
UPDATE users SET profile_image_id = 2  WHERE id = 2;
UPDATE users SET profile_image_id = 3  WHERE id = 3;
UPDATE users SET profile_image_id = 4  WHERE id = 4;
UPDATE users SET profile_image_id = 5  WHERE id = 5;
UPDATE users SET profile_image_id = 6  WHERE id = 6;
